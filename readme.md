...
本仓库为Faster R-CNN的导出onnx的代码，以及配套的推理代码。目标是打通mmdet训练+导出+推理的流程，锁死版本可复现。

建立该仓库的原因在于：

1. 推理代码与训练逻辑高度耦合，不利于直观地理解算法的执行流程;

2. 导出二阶段模型时会使用了自定义算子，比较复杂，不利于原型开发。

![流程图](asset/1.jpg)

## 配置环境

该环境主要用于配合[mmdet](https://github.com/open-mmlab/mmdetection/tree/v2.28.2)进行模型导出。Python 版本选择较为稳定的 3.8 版本环境。
